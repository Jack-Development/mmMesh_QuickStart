...
class SMPL(torch.nn.Module):
    def __init__(self, gender, gpu_ids=[0]):
        ...
        self.device = torch.device(
            "cuda:{}".format(gpu_ids[0]) if torch.cuda.is_available() else "cpu"
        )
        for name in [
            "J_regressor",
            "joint_regressor",
    ...
    def rodrigues(rvec):
        ...
        i_cube = (
            ...
            + torch.zeros((theta_dim, 3, 3), dtype=torch.float32)
        ).to(r.device)
    ...
    def pack(x):
        ...
        :return: (batch, _, 4, 4)
        """
        zeros = torch.zeros((x.shape[0], x.shape[1], 4, 3), dtype=torch.float32).to(
            x.device
        )
    ...
    def forward(self, betas, pose, trans, simplify=False):
        ...
        v_shaped = (
            torch.tensordot(betas, self.shapedirs, dims=([1], [2])) + self.v_template
        )
        ...
        if simplify:
            ...
            I_cube = (
                torch.eye(3, dtype=torch.float32).unsqueeze(dim=0)
                + torch.zeros((batch, R_cube.shape[1], 3, 3), dtype=torch.float32)
            ).to(self.device)
        ...
        results.append(
            self.with_zero(
                torch.cat([R_cube_big[:, 0], J[:, 0, :].reshape(-1, 3, 1)], dim=2)
            )
        )
        for i in range(1, self.kintree_table.shape[1]):
            res = torch.matmul(
        ...
        results = stacked - self.pack(
            torch.matmul(stacked, torch.cat([J, zeros], dim=2).reshape(batch, 24, 4, 1))
        )
        T = torch.tensordot(results, self.weights, dims=([1], [1])).permute(0, 3, 1, 2)
        ones = torch.ones((batch, v_posed.shape[1], 1), dtype=torch.float32).to(
            self.device
        )
        ...
        joints = torch.tensordot(
            mesh_v, self.joint_regressor, dims=([1], [1])
        ).transpose(1, 2)
