...
class data:
    def __init__(self, batch_size, batch_length, pc_size):
        ...
        self.pquat = np.load("./pmat.mcl.20p8a.dat")[
            ..., self.pquat_sample_list, :, :
        ].reshape((20, 8, 3000, 9, 3, 3))
        ...
        self.task_next = multiprocessing.Process(
            target=self.maintain_next_batch, args=(True,)
        )
        self.task_list = []
        for i in range(self.next_extra_process_size):
            self.task_list.append(
                multiprocessing.Process(target=self.maintain_next_batch, args=(False,))
            )
            ...
            for act_no in range(self.act_size):
                ...
                for j in range(self.test_length_size):
                    ...
                    if pc_no < self.pc_size:
                        fill_list = np.random.choice(
                            self.pc_size, size=pc_no, replace=False
                        )
                        fill_set = set(fill_list)
                        pc_tmp[fill_list] = self.test_pc[name_no][act_no][j]
                        dupl_list = [
                            x for x in range(self.pc_size) if x not in fill_set
                        ]
                        dupl_pc = np.random.choice(
                            pc_no, size=len(dupl_list), replace=True
                        )
                        pc_tmp[dupl_list] = self.test_pc[name_no][act_no][j][dupl_pc]
                    else:
                        pc_list = np.random.choice(
                            pc_no, size=self.pc_size, replace=False
                        )
    ...
    def prepare_next_batch(self):
        ...
        batch_betas_list = []
        batch_gender_list = []
        length_list = np.random.choice(
            self.train_length_size - self.batch_length + 1,
        ...
        for i in range(self.batch_size):
            ...
            batch_trans_list.append(
                self.train_trans[
                    name_choice,
                    act_choice,
                    length_begin : length_begin + self.batch_length,
                ]
            )
            batch_pquat_list.append(
                self.train_pquat[
                    name_choice,
                    act_choice,
                    length_begin : length_begin + self.batch_length,
            ...
            batch_betas_list.append(
                np.tile([self.betas[name_choice]], (self.batch_length, 1))
            )
            batch_gender_list.append(
                np.tile([self.gender[name_choice]], (self.batch_length, 1))
            )
            pc_tmp_list = []
            for j in range(length_begin, length_begin + self.batch_length):
                ...
                if pc_no < self.pc_size:
                    ...
                    pc_tmp[dupl_list] = self.train_pc[name_choice][act_choice][j][
                        dupl_pc
                    ]
                else:
        ...
        return (
            np.asarray(batch_pc_list),
            np.asarray(batch_pquat_list),
            np.asarray(batch_trans_list),
            np.asarray(batch_betas_list),
            np.asarray(batch_gender_list),
        )
