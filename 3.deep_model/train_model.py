...
class mmwave:
    ...
    def angle_loss(self, pred_ske, true_ske):
        ...
        angle = torch.sum(
            torch.abs(
                torch.acos(torch.clamp(cos_sim, min=-1.0, max=1.0))
                / 3.14159265358
                * 180.0
            )
        )
    ...
    def cal_vs_from_qtbg(
        ...
        with torch.no_grad():
            ...
            if male_flag.any().item():
                vertice_tensor[male_flag], ske_tensor[male_flag] = self.male_smpl(
            ...
            if female_flag.any().item():
                vertice_tensor[female_flag], ske_tensor[female_flag] = self.female_smpl(
    ...
    def train_once(self):
        ...
        for i in range(self.batch_rate):
            ...
            pred_q, pred_t, pred_v, pred_s, pred_l, pred_b, pred_g, _, _, _, _, _, _ = (
                self.model(pc_tensor, gender_tensor, h0_g, c0_g, h0_a, c0_a)
            )
        ...
        with torch.no_grad():
            ...
            for _ in range(self.train_eval_size):
                ...
                pquat_tensor = torch.tensor(
                    pquat, dtype=torch.float32, device=self.device
                )
                trans_tensor = torch.tensor(
                    trans, dtype=torch.float32, device=self.device
                )
                betas_tensor = torch.tensor(
                    betas, dtype=torch.float32, device=self.device
                )
                gender_tensor = torch.tensor(
                    gender, dtype=torch.float32, device=self.device
                )
                ...
                train_trans_report += torch.sum(
                    torch.sqrt(torch.sum(torch.square(pred_t - trans_tensor), dim=-1))
                ).item()
                train_vertice_report += torch.sum(
                    torch.sqrt(torch.sum(torch.square(pred_v - vertice_tensor), dim=-1))
                ).item()
                train_ske_report += torch.sum(
                    torch.sqrt(torch.sum(torch.square(pred_s - ske_tensor), dim=-1))
                ).item()
                train_loc_report += torch.sum(
                    torch.sqrt(
                        torch.sum(torch.square(pred_l - trans_tensor[..., :2]), dim=-1)
                    )
                ).item()
            ...
            for name_no in range(self.name_size):
                betas_tensor = torch.tensor(
                    np.expand_dims(self.dataset.betas[name_no : name_no + 1], 0),
                    dtype=torch.float32,
                    device=self.device,
                )
                gender_tensor = torch.tensor(
                    np.expand_dims(self.dataset.gender[name_no : name_no + 1], 0),
                    dtype=torch.float32,
                    device=self.device,
                )
                for act_no in range(self.act_size):
                    pc_tensor = torch.tensor(
                        np.expand_dims(
                            np_pc[name_no, act_no, : self.test_length_seg], 0
                        ),
                        dtype=torch.float32,
                        device=self.device,
                    )
                    pquat_tensor = torch.tensor(
                        np.expand_dims(
                            np_pquat[name_no, act_no, : self.test_length_seg], 0
                        ),
                        dtype=torch.float32,
                        device=self.device,
                    )
                    trans_tensor = torch.tensor(
                        np.expand_dims(
                            np_trans[name_no, act_no, : self.test_length_seg], 0
                        ),
                        dtype=torch.float32,
                        device=self.device,
                    )
                    vertice_tensor, ske_tensor = self.cal_vs_from_qtbg(
                        pquat_tensor,
                        trans_tensor,
                        betas_tensor.repeat(1, self.test_length_seg, 1),
                        gender_tensor.repeat(1, self.test_length_seg, 1),
                        1,
                        self.test_length_seg,
                    )
                    h0_g = torch.zeros(
                        (3, 1, 64), dtype=torch.float32, device=self.device
                    )
                    c0_g = torch.zeros(
                        (3, 1, 64), dtype=torch.float32, device=self.device
                    )
                    ...
                    (
                        ...
                        hn_a,
                        cn_a,
                    ) = self.model(pc_tensor, None, h0_g, c0_g, h0_a, c0_a)
                    ...
                    betas_loss += self.criterion(
                        pred_b, betas_tensor.repeat(1, self.test_length_seg, 1)
                    ).item()
                    gender_loss += self.criterion_gender(
                        pred_g, gender_tensor.repeat(1, self.test_length_seg, 1)
                    ).item()

                    angle_report += self.angle_loss(pred_s, ske_tensor)
                    trans_report += torch.sum(
                        torch.sqrt(
                            torch.sum(torch.square(pred_t - trans_tensor), dim=-1)
                        )
                    ).item()
                    vertice_report += torch.sum(
                        torch.sqrt(
                            torch.sum(torch.square(pred_v - vertice_tensor), dim=-1)
                        )
                    ).item()
                    ske_report += torch.sum(
                        torch.sqrt(torch.sum(torch.square(pred_s - ske_tensor), dim=-1))
                    ).item()
                    loc_report += torch.sum(
                        torch.sqrt(
                            torch.sum(
                                torch.square(pred_l - trans_tensor[..., :2]), dim=-1
                            )
                        )
                    ).item()
                    ...
                    gender_acc += (
                        1.0
                        - torch.sum(torch.abs(pred_g - gender_tensor)).item()
                        / self.test_length_seg
                    ) / self.test_seg
                    for seg_no in range(1, self.test_seg):
                        ...
                        (
                            ...
                            hn_a,
                            cn_a,
                        ) = self.model(pc_tensor, None, hn_g, cn_g, hn_a, cn_a)
                        ...
                        ske_report += torch.sum(
                            torch.sqrt(
                                torch.sum(torch.square(pred_s - ske_tensor), dim=-1)
                            )
                        ).item()
                        loc_report += torch.sum(
                            torch.sqrt(
            ...
            vertice_report /= (
                self.name_size * self.act_size * self.test_length_size * 6890
            )
    ...
    def train_model(self):
        ...
        evalfile.flush()
        print(
            "%6d || %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f || %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f"
            % (
                0,
                train_pquat_loss,
                train_trans_loss,
        ...
        for i in range(self.train_size):
            ...
            if (i + 1) % self.write_slot == 0:
                ...
                print(
                    "%6d || %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f || %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f | %7.3f || %5.2f / %5.2f"
                    % (
                        i + 1,
                        train_pquat_loss,
                        train_trans_loss,
    ...
    def load_model(self, name):
        self.model.load_state_dict(
            torch.load("./%s/model/" % (V_NO) + name + ".pth", map_location=self.device)
        )
