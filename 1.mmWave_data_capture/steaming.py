...
ADC_PARAMS = {"chirps": 128, "rx": 4, "tx": 3, "samples": 256, "IQ": 2, "bytes": 2}
# STATIC
...
class adcCapThread(threading.Thread):
    ...
    def _frame_receiver(self):
        ...
        while self.whileSign:
            ...
            if after_packet_count < BYTES_IN_PACKET:
                recentframe[0 : after_packet_count // 2] = packet_data[
                    (BYTES_IN_PACKET - after_packet_count) // 2 :
                ]
        ...
        while self.whileSign:
            ...
            if recentframe_collect_count + BYTES_IN_PACKET >= BYTES_IN_FRAME:
                recentframe[recentframe_collect_count // 2 :] = packet_data[
                    : (BYTES_IN_FRAME - recentframe_collect_count) // 2
                ]
    ...
    def _store_frame(self, recentframe):
        ...
        if (
            self.nextReadBufferPosition - 1 + self.bufferSize
        ) % self.bufferSize == self.nextCapBufferPosition:
            self.bufferOverWritten = True
